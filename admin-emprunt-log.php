...
<head>
  ...
  <title>The Reading Room - Emprunt Log</title>
...
$("#search-input").on("input", function() {
    ...
    $("#reservation-table tbody tr").each(function() {
        let logId = $(this).find("td:nth-child(1)").text().toLowerCase();
        let ouvreTitre = $(this).find("td:nth-child(2)").text().toLowerCase();
        let nomAdherent = $(this).find("td:nth-child(6)").text().toLowerCase();
        let bibNom = $(this).find("td:nth-child(7)").text().toLowerCase();
        if (logId.includes(searchTerm) || ouvreTitre.includes(searchTerm) || nomAdherent.includes(searchTerm) || bibNom.includes(searchTerm)) {
            $(this).show();
        } else {
            $(this).hide();
        }
